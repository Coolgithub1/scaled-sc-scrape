...
**Source of truth audited:** `boza_scraper/boza_members.csv`  
**Original snapshot:** 264 rows, 32 of 46 counties, 208 sitting / 56 historical (2026-09-04)  
**After repair:** see `boza_scraper/county_coverage.csv` and the remediation section below.

## Remediation status (2026-09-05)

Fixed in the scraper and/or repaired CSV:

- Calhoun Planning Commission rows removed (5 real BZA sitting remain).
- Vacant seats recorded for Abbeville D1/D3, Anderson D4, Chester D1 + At Large, Florence D7/D9, Lexington D1/D5, Richland unnamed 7th, McCormick unpublished 5th.
- Marion sitting roster filled (7 names from the county planning page).
- Beaufort historic names inverted to First Last.
- Orangeburg: Herbert Sellers (not Hebert); Yvonne Gooden-Johnson; William Weathers restored to sitting.
- Chester Melvin B. Jackson merged; terms 2025–2028.
- Pickens Bob Fetterly restored to sitting.
- Sumter Claude Wheeler moved to historical; official board is 9.
- Attendance-only years cleared from `term_start`/`term_end` (Aiken, Charleston, Kershaw, Fairfield, etc.).
- Phones, emails, and street addresses stripped from `tenure`.
- Gender filled on every non-vacant row.
- Historic crawl caps raised (8 docs/year, keep 80, scan 200).
- `--county` no longer overwrites the statewide CSV.
- Combined PC+BZA pages skip Planning Commission tables; vacant seats are kept.

Honestly marked, not invented:

- Laurens, Union, Cherokee, Saluda, Williamsburg: no county zoning BZA.
- Allendale, Bamberg, Barnwell, Chesterfield, Dillon, Lee, Marlboro: board may exist; no public names.
- Hampton published Board of Adjustments is building-codes, not ingested as BZA.
- Edgefield official size is 3; Jasper published size is 4.
- Aiken Jason Whinghter / Doug Engebrethson, Charleston AD Jordan, Richland Anette Nelson are the published spellings.
